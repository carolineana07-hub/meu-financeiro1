...
function renderListaPessoasDividas(){
  ...
  list.innerHTML = `<div class="grid" style="grid-template-columns:repeat(2,1fr);">` + pessoas.map(pessoa=>{
    ...
    // Só a(s) parcela(s) que vencem ESTE mês para cada compra — nunca a soma das parcelas futuras
    const parcelasEsteMes = todasParcelas.filter(p=> idsDividasPessoa.includes(p.dividaId) && p.status!=='paga' && dataDentroDoPeriodo(p.vencimento, periodoMesAtual));
    const valorEsteMes = parcelasEsteMes.reduce((s,p)=>s+(p.valor||0),0);
    ...
    return `<div class="card" data-pessoa="${pessoa}" style="cursor:pointer; gap:4px;">
      ...
      <div style="font-size:9.5px; color:var(--plum-soft); font-weight:700; text-transform:uppercase; letter-spacing:0.03em; margin-top:-4px;">Este mês${parcelasEsteMes.length>1?' · '+parcelasEsteMes.length+'x':''}</div>
