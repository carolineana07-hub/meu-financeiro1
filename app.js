...
function renderListaContasFixas(){
  ...
  list.innerHTML = contasOrdenadas.map(cf=>{
    ...
    <button data-toggle-fixa="${cf.id}" style="margin-top:8px; width:100%; background:var(--stone); border:none; border-radius:8px; padding:7px; font-size:11.5px; font-weight:700; color:var(--plum-ink); cursor:pointer;">${expandida?'▲ Ocultar detalhes':'▶ Ver detalhes'}</button>`;
    ...
    if(expandida){
      ...
      detalhesHtml = `<div style="width:100%; margin-top:10px; padding-top:10px; border-top:1px dashed var(--stone-dark);">
        ...
        <div style="display:flex; gap:8px; margin-top:10px;">
          <button class="tx-del" data-id="${cf.id}" data-action="editar" style="flex:1; background:var(--stone); border-radius:8px; padding:8px; font-size:11.5px; font-weight:700;" title="Editar">✏️ Editar</button>
          <button class="tx-del" data-id="${cf.id}" data-action="excluir" style="flex:1; background:var(--stone); border-radius:8px; padding:8px; font-size:11.5px; font-weight:700; color:var(--red);" title="Excluir">🗑️ Excluir</button>
        </div>
      </div>`;
    }
