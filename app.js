...
function renderListaDividas(){
  ...
  // Agrupar por pessoa, cada uma na sua própria "caixinha", em ordem alfabética
  const pessoas = [...new Set(dividas.map(d=>d.pessoa))].sort((a,b)=>a.localeCompare(b,'pt-BR'));

  list.innerHTML = pessoas.map(pessoa=>{
    const dividasDaPessoa = dividas.filter(d=>d.pessoa===pessoa);
    const totalRestantePessoa = dividasDaPessoa.reduce((s,d)=>s+restanteEPagoDivida(d, todasParcelas).restante,0);

    const itensHtml = dividasDaPessoa.map(d=>{
      const {restante, pago} = restanteEPagoDivida(d, todasParcelas);
      ...
      let parcelasHtml = '';
      if(d.parcelado){
        const parcelas = todasParcelas.filter(p=>p.dividaId===d.id).sort((a,b)=> new Date(a.vencimento)-new Date(b.vencimento));
      ...
      return `<div class="tx-item" style="flex-wrap:wrap;">
        <div class="tx-left">
          <div class="tx-emoji">${d.tipo==='devo'?'📤':'📥'}</div>
          <div class="tx-info">
        ...
        <div class="tx-right" style="flex-direction:column; align-items:flex-end; gap:4px;">
          <div class="tx-value ${quitada?'pos':'neg'}" style="display:flex; align-items:center; gap:6px;">
            ${quitada?'Quitada':money(restante)+' rest.'}
            ${(!quitada && !d.parcelado)?`<button data-editar-total="${d.id}" title="Editar valor" style="font-size:11px; background:none; border:none; cursor:pointer;">✏️</button>`:''}
          </div>
    ...
    return `<div style="margin-bottom:18px;">
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
        <strong style="font-family:'Fraunces',serif; font-size:15px;">${pessoa}</strong>
        <span style="font-size:12.5px; font-weight:700; color:var(--plum-soft);">${money(totalRestantePessoa)}</span>
      </div>
      ${itensHtml}
    </div>`;
  }).join('');
  ...
  list.querySelectorAll('[data-parcela-divida-toggle]').forEach(btn=>{
    btn.addEventListener('click', ()=> toggleParcelaDividaPaga(btn.dataset.parcelaDividaToggle));
  });
  list.querySelectorAll('[data-parcela-divida-editar]').forEach(btn=>{
    btn.addEventListener('click', ()=> editarValorParcelaDivida(btn.dataset.parcelaDividaEditar));
  });
  list.querySelectorAll('[data-editar-total]').forEach(btn=>{
    btn.addEventListener('click', ()=> editarValorTotalDivida(btn.dataset.editarTotal));
  });
}

function toggleParcelaDividaPaga(parcelaId){
...
function editarValorParcelaDivida(parcelaId){
  const parcelas = DB.get('parcelas_divida');
  const p = parcelas.find(x=>x.id===parcelaId);
  if(!p) return;
...
function editarValorTotalDivida(dividaId){
  const dividas = DB.get('dividas');
  const d = dividas.find(x=>x.id===dividaId);
  if(!d) return;
  const novoValorStr = prompt('Novo valor total da dívida (R$):', d.valorTotal);
  if(novoValorStr===null) return;
  const novoValor = parseFloat(novoValorStr.replace(',','.'));
  if(!novoValor || novoValor<=0) return;
  d.valorTotal = novoValor;
