...
function renderListaPessoasDividas(){
  ...
  list.innerHTML = `<div class="grid" style="grid-template-columns:repeat(2,1fr);">` + pessoas.map(pessoa=>{
    ...
    return `<div class="card" data-pessoa="${pessoa}" style="cursor:pointer; gap:4px;">
      ...
      <div class="tx-value neg" style="font-size:15px;">${money(r.totalAberto)}</div>
      <div style="font-size:10.5px; color:var(--plum-soft); font-weight:600;">🧾 ${r.comprasAberto} · ⏳ ${r.parcelasPendentes}</div>
      <div style="font-size:10.5px; color:var(--plum-soft); font-weight:600;">📅 ${vencTxt}</div>
      ${parcelasNoPeriodo.length>0 ? `<div style="font-size:10px; font-weight:700; color:var(--rose-deep);">🗓️ ${parcelasNoPeriodo.length}x no período</div>` : ''}
    </div>`;
  }).join('') + `</div>`;
