...
/* Status padrão usado em qualquer parcela do app: 🟢 Pago, 🟡 Pendente
   (vence neste mês), 🔴 Vencido, 🔵 Futuro (vence depois deste mês). */
function statusParcelaPadrao(vencimentoStr, pago){
  if(pago) return {emoji:'🟢', label:'Pago'};
  const hoje = new Date(); hoje.setHours(0,0,0,0);
  const venc = new Date(vencimentoStr+'T00:00:00');
  const fimMesAtual = new Date(hoje.getFullYear(), hoje.getMonth()+1, 0);
  if(venc < hoje) return {emoji:'🔴', label:'Vencido'};
  if(venc <= fimMesAtual) return {emoji:'🟡', label:'Pendente'};
  return {emoji:'🔵', label:'Futuro'};
}
...
function calcularDashboard(periodoParam){
  ...
  // Terceiros: quanto está previsto entrar/sair NESTE período (parcelas de dívida
  // com vencimento no período, incluindo dívidas sem parcelamento pelo próprio vencimento)
  function previstoNoPeriodo(tipoDivida){
    let total = 0;
    dividas.filter(d=>d.tipo===tipoDivida).forEach(d=>{
      if(d.parcelado){
        parcelasDivida.filter(p=>p.dividaId===d.id && p.status!=='paga' && dataDentroDoPeriodo(p.vencimento, periodo)).forEach(p=>{ total += p.valor||0; });
      } else {
        const {restante} = restanteEPagoDivida(d, parcelasDivida);
        const vencRef = d.vencimento || d.data;
        if(restante>0 && vencRef && dataDentroDoPeriodo(vencRef, periodo)) total += restante;
      }
    });
    return total;
  }
  const previstoReceberPeriodo = previstoNoPeriodo('me_devem');
  const previstoPagarPeriodo = previstoNoPeriodo('devo');
...
function renderDashboard(){
  ...
  try{ renderSaudeFinanceira(d); }catch(err){ console.error('Falha ao renderizar Saúde Financeira:', err); }
...
function excluirTransacao(id){
  ...
  // Se era uma compra parcelada, remove as parcelas geradas por ela também
  DB.save('parcelas', DB.get('parcelas').filter(p=>p.transacaoOrigemId!==id));
...
function corDoBanco(banco){
  ...
  if(n.includes('nubank')) return {grad:'linear-gradient(135deg, #9B2FD6, #57068C)', texto:'#fff'};
  if(n.includes('mercado')) return {grad:'linear-gradient(135deg, #2E2E33, #000000)', texto:'#fff'};
  if(n.includes('caixa')) return {grad:'linear-gradient(135deg, #1E5FC7, #002D72)', texto:'#fff'};
  if(n.includes('ita')) return {grad:'linear-gradient(135deg, #FF8C2E, #C15A00)', texto:'#fff'};
  if(n.includes('banco do brasil') || n==='bb') return {grad:'linear-gradient(135deg, #FCD535, #E0A800)', texto:'#4A3A42'};
  if(n.includes('santander')) return {grad:'linear-gradient(135deg, #EC0000, #8C0000)', texto:'#fff'};
  if(n.includes('bradesco')) return {grad:'linear-gradient(135deg, #CC092F, #7A0016)', texto:'#fff'};
  if(n.includes('inter')) return {grad:'linear-gradient(135deg, #FF7A00, #E85D00)', texto:'#fff'};
  if(n.includes('c6')) return {grad:'linear-gradient(135deg, #3A3A3A, #000000)', texto:'#fff'};
  if(n.includes('picpay')) return {grad:'linear-gradient(135deg, #21C25E, #0C7A38)', texto:'#fff'};
  if(n.includes('next')) return {grad:'linear-gradient(135deg, #00FF5F, #00B347)', texto:'#0A2E1A'};
  if(n.includes('will')) return {grad:'linear-gradient(135deg, #FF5031, #B5300E)', texto:'#fff'};
  return {grad:'linear-gradient(135deg, var(--plum-ink), #6B5560)', texto:'var(--white)'};
}
...
function renderListaCartoes(){
  ...
  list.innerHTML = cartoes.map(c=>{
    ...
    const cor = corDoBanco(c.banco);
    return `<div class="card-visual" style="background:${cor.grad}; color:${cor.texto};">
      <div class="cv-top">
        <span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; min-width:0;">${isDebito?'🏧 Débito':'💳 Crédito'} · ${c.banco}</span>
        <div style="display:flex; align-items:center; gap:6px; flex-shrink:0;">
          <span>${c.bandeira}</span>
          <button data-id="${c.id}" data-action="editar" title="Editar" style="background:rgba(255,255,255,0.18); border:none; color:inherit; width:24px; height:24px; border-radius:50%; cursor:pointer; font-size:11px; flex-shrink:0;">✏️</button>
          <button data-id="${c.id}" data-action="excluir" title="Excluir" style="background:rgba(255,255,255,0.18); border:none; color:inherit; width:24px; height:24px; border-radius:50%; cursor:pointer; font-size:11px; flex-shrink:0;">✕</button>
        </div>
      </div>
  ...
  list.querySelectorAll('[data-action="excluir"]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      if(!confirm('Excluir este cartão? As parcelas em aberto dele também serão removidas.')) return;
      DB.save('cartoes', DB.get('cartoes').filter(c=>c.id!==btn.dataset.id));
      DB.save('parcelas', DB.get('parcelas').filter(p=>p.cartaoId!==btn.dataset.id));
      renderListaCartoes();
      renderDashboard();
      renderNotificacoes();
    });
...
function abrirModalContaFixa(contaFixaId){
  ...
  document.getElementById('cfPrimeiraData').value = (cf && cf.primeiraData) ? cf.primeiraData : new Date().toISOString().slice(0,10);
  document.getElementById('cfNumParcelas').value = (cf && cf.numParcelas) ? cf.numParcelas : 12;
...
document.getElementById('btnSalvarContaFixa').addEventListener('click', ()=>{
  ...
  contaFixaEditandoId = null;
  fecharModal('modalContaFixa');
  renderListaContasFixas();
  renderDashboard();
});

let contasFixasExpandidas = new Set();
...
function renderListaContasFixas(){
  ...
  const pendentesGeral = todasParcelas.filter(p=>p.status!=='pago');
  const totalPendenteGeral = pendentesGeral.reduce((s,p)=>s+p.valor,0);
  resumoPanel.innerHTML = contasFixas.length===0 ? '' : `
    <div class="label" style="margin-bottom:6px;">${periodo.label}</div>
    <div class="resumo-item"><span class="tag">🧾 Total do período</span><span class="amt">${money(totalPeriodo)}</span></div>
    <div class="resumo-item"><span class="tag">🟢 Pagas no período</span><span class="amt">${pagasPeriodo} de ${parcelasNoPeriodo.length}</span></div>
    <div class="resumo-item"><span class="tag">🧮 Contas cadastradas</span><span class="amt">${contasFixas.length}</span></div>
    <div class="resumo-item"><span class="tag">⏳ Parcelas pendentes (total)</span><span class="amt">${pendentesGeral.length} · ${money(totalPendenteGeral)}</span></div>
  `;
  ...
  list.innerHTML = contasOrdenadas.map(cf=>{
    ...
    const totalRestante = pendentes.reduce((s,p)=>s+p.valor,0);
    const expandida = contasFixasExpandidas.has(cf.id);
    const st = proxima ? statusParcelaPadrao(proxima.vencimento, false) : {emoji:'🟢', label:'Tudo pago'};

    // ---- Card resumido (sempre visível) ----
    const resumoHtml = `<div style="display:flex; align-items:center; justify-content:space-between; width:100%;">
      <div class="tx-left">
        <div class="tx-emoji">${meta?meta.emoji:'📅'}</div>
        <div class="tx-info">
          <div class="tx-desc">${cf.nome}</div>
          <div class="tx-meta">${st.emoji} ${proxima ? 'Próxima: '+new Date(proxima.vencimento+'T00:00:00').toLocaleDateString('pt-BR')+' · '+money(proxima.valor) : 'Tudo pago'}</div>
        </div>
      </div>
      <div class="tx-right" style="flex-direction:column; align-items:flex-end; gap:2px;">
        <div style="display:flex; gap:4px; margin-bottom:2px;">
          <button data-id="${cf.id}" data-action="editar" title="Editar conta" style="background:var(--stone); border:none; width:26px; height:26px; border-radius:50%; cursor:pointer; font-size:12px;">✏️</button>
          <button data-id="${cf.id}" data-action="excluir" title="Excluir conta" style="background:var(--stone); border:none; width:26px; height:26px; border-radius:50%; cursor:pointer; font-size:12px;">🗑️</button>
        </div>
        <div class="tx-value neg" style="font-size:13px;">${money(totalRestante)}</div>
        <div style="font-size:11px; color:var(--plum-soft); font-weight:600;">${pendentes.length} parcela${pendentes.length!==1?'s':''}</div>
      </div>
    </div>
    <button data-toggle-fixa="${cf.id}" style="margin-top:8px; width:100%; background:var(--stone); border:none; border-radius:8px; padding:7px; font-size:11.5px; font-weight:700; color:var(--plum-ink); cursor:pointer;">${expandida?'▲ Ocultar parcelas':'▶ Ver parcelas'}</button>`;

    // ---- Detalhes (só quando expandido) ----
    let detalhesHtml = '';
    if(expandida){
      const parcelasRows = parcelas.map(p=>{
        const ps = statusParcelaPadrao(p.vencimento, p.status==='pago');
        return `<div style="display:flex; align-items:center; justify-content:space-between; font-size:12.5px; padding:6px 0; border-bottom:1px solid var(--stone);">
          <span>${ps.emoji} Parcela ${p.numero}/${p.totalParcelas} · ${new Date(p.vencimento+'T00:00:00').toLocaleDateString('pt-BR')}</span>
          <span style="display:flex; align-items:center; gap:6px;">
            ...
            <button data-parcela-fixa="${p.id}" style="font-size:11px; color:var(--rose-deep); font-weight:700; background:none; border:none; cursor:pointer;">${p.status==='pago'?'Desfazer':'Marcar paga'}</button>
          </span>
        </div>`;
      }).join('');
      detalhesHtml = `<div style="width:100%; margin-top:10px; padding-top:10px; border-top:1px dashed var(--stone-dark);">
        <div style="max-height:220px; overflow-y:auto; display:flex; flex-direction:column;">${parcelasRows}</div>
      </div>`;
    }

    return `<div class="tx-item" style="flex-wrap:wrap; display:block; padding:14px 0;">${resumoHtml}${detalhesHtml}</div>`;
  }).join('');

  list.querySelectorAll('[data-toggle-fixa]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const id = btn.dataset.toggleFixa;
      if(contasFixasExpandidas.has(id)) contasFixasExpandidas.delete(id); else contasFixasExpandidas.add(id);
      renderListaContasFixas();
    });
  });
  list.querySelectorAll('[data-action="editar"]').forEach(btn=>{
...
let filtroDividaAtual = 'me_devem';
...
function renderListaPessoasDividas(){
  ...
  const todasParcelas = DB.get('parcelas_divida');
  // Mês corrente de verdade — independente do período selecionado no topo,
  // porque "quanto vence esse mês" deve ser sempre o calendário real.
  const hoje = new Date();
  const periodoMesAtual = { inicio: new Date(hoje.getFullYear(), hoje.getMonth(), 1), fim: new Date(hoje.getFullYear(), hoje.getMonth()+1, 0, 23,59,59,999) };

  list.innerHTML = `<div class="grid" style="grid-template-columns:repeat(2,1fr);">` + pessoas.map(pessoa=>{
    const r = dadosResumoPessoa(pessoa, filtroDividaAtual);
    const dividasPessoa = dividas.filter(d=>d.pessoa===pessoa);
    // Só o que vence ESTE mês conta aqui — cada compra parcelada entra só com a(s)
    // parcela(s) pendente(s) do mês; compra sem parcelamento entra com o valor
    // restante se o vencimento dela cair neste mês.
    let parcelasEsteMesCount = 0, valorEsteMes = 0;
    dividasPessoa.forEach(d=>{
      if(d.parcelado){
        todasParcelas.filter(p=>p.dividaId===d.id && p.status!=='paga' && dataDentroDoPeriodo(p.vencimento, periodoMesAtual)).forEach(p=>{
          parcelasEsteMesCount++; valorEsteMes += p.valor||0;
        });
      } else {
        const {restante} = restanteEPagoDivida(d, todasParcelas);
        const vencRef = d.vencimento || d.data;
        if(restante>0 && vencRef && dataDentroDoPeriodo(vencRef, periodoMesAtual)){
          parcelasEsteMesCount++; valorEsteMes += restante;
        }
      }
    });
    if(r.comprasAberto===0){
      return `<div class="card" data-pessoa="${pessoa}" style="cursor:pointer; opacity:0.6;">
        <div style="font-weight:700; font-size:13px;">${pessoa}</div>
        <div style="font-size:11.5px; color:var(--green); font-weight:700;">✅ Quitado</div>
      </div>`;
    }
    const vencTxt = r.proximoVencimento ? new Date(r.proximoVencimento+'T00:00:00').toLocaleDateString('pt-BR') : '—';
    return `<div class="card" data-pessoa="${pessoa}" style="cursor:pointer; gap:4px;">
      <div style="display:flex; align-items:center; justify-content:space-between;">
        <div style="font-weight:700; font-size:13.5px; font-family:'Fraunces',serif;">${pessoa}</div>
        <span style="font-size:11px; color:var(--plum-soft);">›</span>
      </div>
      <div class="tx-value neg" style="font-size:15px;">${valorEsteMes>0 ? money(valorEsteMes) : '—'}</div>
      <div style="font-size:9.5px; color:var(--plum-soft); font-weight:700; text-transform:uppercase; letter-spacing:0.03em; margin-top:-4px;">Este mês${parcelasEsteMesCount>1?' · '+parcelasEsteMesCount+'x':''}</div>
      <div style="font-size:10.5px; color:var(--plum-soft); font-weight:600; margin-top:2px;">🧾 ${r.comprasAberto} compra${r.comprasAberto!==1?'s':''} · Total em aberto: ${money(r.totalAberto)}</div>
      <div style="font-size:10.5px; color:var(--plum-soft); font-weight:600;">📅 Próx.: ${vencTxt}</div>
    </div>`;
  }).join('') + `</div>`;
...
function renderComprasPessoa(pessoa){
  ...
  list.innerHTML = compras.map(d=>{
    ...
    const st = quitada ? {emoji:'🟢'} : (d.parcelado ? statusParcelaPadrao(proxima ? proxima.vencimento : d.data, false) : statusParcelaPadrao(d.vencimento || d.data, false));
    const pendentesCount = d.parcelado ? (d.numParcelas - pagas) : (quitada?0:1);
    ...
    const resumoHtml = `<div style="display:flex; align-items:center; justify-content:space-between; width:100%;">
      <div class="tx-left">
        ...
        <div class="tx-info">
          <div class="tx-desc">${d.motivo || 'Sem descrição'}</div>
          <div class="tx-meta">${st.emoji} ${quitada ? 'Quitada' : 'Próxima: '+vencTxtCompra+(d.parcelado&&proxima?' · '+money(proxima.valor):'')}</div>
        </div>
      </div>
      <div class="tx-right" style="flex-direction:column; align-items:flex-end; gap:2px;">
        <div class="tx-value ${quitada?'pos':'neg'}" style="font-size:13px;">${quitada?'Quitada':money(restante)}</div>
        <div style="font-size:11px; color:var(--plum-soft); font-weight:600;">${pendentesCount} parcela${pendentesCount!==1?'s':''}</div>
      </div>
    </div>
    <button data-compra-toggle="${d.id}" style="margin-top:8px; width:100%; background:var(--stone); border:none; border-radius:8px; padding:7px; font-size:11.5px; font-weight:700; color:var(--plum-ink); cursor:pointer;">${expandida?'▲ Ocultar detalhes':'▶ Ver detalhes'}</button>`;

    // ---- Detalhes (só quando expandido) ----
    let detalhesHtml = '';
    if(expandida){
      const parcelasRows = d.parcelado ? parcelas.map(p=>{
        const paga = p.status==='paga';
        const ps = statusParcelaPadrao(p.vencimento, paga);
        return `<div style="display:flex; align-items:center; justify-content:space-between; font-size:12.5px; padding:6px 0; border-bottom:1px solid var(--stone);">
          <span>${ps.emoji} Parcela ${p.numero} · ${new Date(p.vencimento+'T00:00:00').toLocaleDateString('pt-BR')}</span>
          <span style="display:flex; align-items:center; gap:6px;">
            <strong>${money(p.valor)}</strong>
      ...
      }).join('') : `<div style="font-size:12.5px; color:var(--plum-soft); padding:6px 0;">Pagamento único${quitada?' · quitado':''}.</div>`;

      detalhesHtml = `<div style="width:100%; margin-top:10px; padding-top:10px; border-top:1px dashed var(--stone-dark);">
        <div style="max-height:220px; overflow-y:auto; display:flex; flex-direction:column;">${parcelasRows}</div>
        <div style="display:flex; gap:8px; margin-top:10px;">
          ${(!d.parcelado && !quitada) ? `<button data-compra-pagar="${d.id}" style="flex:1; background:var(--stone); border-radius:8px; padding:8px; font-size:11.5px; font-weight:700;">💰 Registrar pagamento</button>` : ''}
          <button data-compra-editar="${d.id}" style="flex:1; background:var(--stone); border-radius:8px; padding:8px; font-size:11.5px; font-weight:700;" title="Editar">✏️ Editar</button>
          <button data-compra-excluir="${d.id}" style="flex:1; background:var(--stone); border-radius:8px; padding:8px; font-size:11.5px; font-weight:700; color:var(--red);" title="Excluir">🗑️ Excluir</button>
        </div>
      </div>`;
    }
...
/* ================================================================
   ...
   Cada notificação tem uma "chave" = id do item + status no momento
   (vencida/hoje/amanha). Limpar uma notificação guarda essa chave;
   ela só volta a aparecer se o status mudar (virou vencida depois de
   ter sido "hoje", por exemplo) — isso é um evento novo, então gera
   uma chave nova e a notificação some ao ser resolvida ou limpa e não
   volta a incomodar pelo mesmo motivo.
   ================================================================ */
function coletarAlertas(){
  ...
  DB.get('parcelas').filter(p=>p.status!=='pago').forEach(p=>{
    ...
    if(st){
      const cartao = DB.get('cartoes').find(c=>c.id===p.cartaoId);
      alertas.push({ id:'parc-'+p.id, rawId:p.id, origem:'cartao', status:st, texto:`Parcela ${p.numero}/${p.totalParcelas}${cartao?' · '+cartao.nome:''}`, valor:p.valor });
    }
  });
  DB.get('parcelas_fixas').filter(p=>p.status!=='pago').forEach(p=>{
    ...
    if(st){
      const cf = DB.get('contas_fixas').find(c=>c.id===p.contaFixaId);
      alertas.push({ id:'fixa-'+p.id, rawId:p.id, origem:'fixa', status:st, texto: cf?cf.nome:'Conta fixa', valor:p.valor });
    }
  });
...
function chaveAlerta(a){ return a.id + ':' + a.status; }

function getNotificacoesLimpas(){
  try{ return JSON.parse(localStorage.getItem('financeiro_notif_limpas')||'[]'); }catch(e){ return []; }
}
function salvarNotificacoesLimpas(arr){
  localStorage.setItem('financeiro_notif_limpas', JSON.stringify(arr));
}
function alertasVisiveis(){
  const limpas = new Set(getNotificacoesLimpas());
  return coletarAlertas().filter(a => !limpas.has(chaveAlerta(a)));
}
function limparNotificacao(chave){
  const limpas = getNotificacoesLimpas();
  if(!limpas.includes(chave)) limpas.push(chave);
  salvarNotificacoesLimpas(limpas);
  renderNotificacoes();
}
function limparTodasNotificacoes(){
  const limpas = getNotificacoesLimpas();
  coletarAlertas().forEach(a=>{ const k=chaveAlerta(a); if(!limpas.includes(k)) limpas.push(k); });
  salvarNotificacoesLimpas(limpas);
  renderNotificacoes();
}

const STATUS_LABEL = { vencida:'🔴 Vencida', hoje:'🔴 Vence hoje', amanha:'🟡 Vence amanhã' };

function renderNotificacoes(){
  const alertas = alertasVisiveis();
  ...
  list.innerHTML = `<button id="btnLimparTodasNotif" style="width:100%; background:var(--stone); border:none; border-radius:10px; padding:8px; font-size:11.5px; font-weight:700; color:var(--plum-ink); cursor:pointer; margin-bottom:10px;">Limpar todas</button>` +
    alertas.map(a=>{
      const chave = chaveAlerta(a);
      return `<div style="border-bottom:1px solid var(--stone); padding:10px 0; display:flex; flex-direction:column; gap:6px;">
        <div style="display:flex; justify-content:space-between; gap:8px;">
          <span style="font-size:13px; font-weight:600;">${STATUS_LABEL[a.status]} · ${a.texto}</span>
          <span style="font-weight:700; font-family:'Fraunces',serif;">${money(a.valor)}</span>
        </div>
        <div style="display:flex; gap:6px; flex-wrap:wrap;">
          <button data-notif-pagar data-origem="${a.origem}" data-rawid="${a.rawId}" style="font-size:11px; background:var(--stone); border:none; border-radius:8px; padding:5px 9px; cursor:pointer; font-weight:700; color:var(--plum-ink);">✅ Marcar paga</button>
          <button data-notif-adiar data-origem="${a.origem}" data-rawid="${a.rawId}" style="font-size:11px; background:var(--stone); border:none; border-radius:8px; padding:5px 9px; cursor:pointer; font-weight:700; color:var(--plum-ink);">📅 Adiar</button>
          <button data-notif-limpar="${chave}" style="font-size:11px; background:none; border:none; padding:5px 9px; cursor:pointer; font-weight:700; color:var(--plum-soft);">✕ Limpar</button>
        </div>
      </div>`;
    }).join('');

  document.getElementById('btnLimparTodasNotif').addEventListener('click', limparTodasNotificacoes);
  list.querySelectorAll('[data-notif-pagar]').forEach(btn=>{
    btn.addEventListener('click', ()=> marcarAlertaComoPago(btn.dataset.origem, btn.dataset.rawid));
  });
  list.querySelectorAll('[data-notif-adiar]').forEach(btn=>{
    btn.addEventListener('click', ()=> adiarAlerta(btn.dataset.origem, btn.dataset.rawid));
  });
  list.querySelectorAll('[data-notif-limpar]').forEach(btn=>{
    btn.addEventListener('click', ()=> limparNotificacao(btn.dataset.notifLimpar));
  });
}

function marcarAlertaComoPago(origem, rawId){
  if(origem==='cartao'){
    const parcelas = DB.get('parcelas');
    const p = parcelas.find(x=>x.id===rawId);
    if(p){ p.status='pago'; DB.save('parcelas', parcelas); }
    renderNotificacoes();
    renderDashboard();
    if(currentView==='cartoes') renderListaCartoes();
  } else if(origem==='fixa'){
    toggleParcelaFixaPaga(rawId);
  }
}

function adiarAlerta(origem, rawId){
  const novaData = prompt('Adiar para qual data? (AAAA-MM-DD)', new Date().toISOString().slice(0,10));
  if(!novaData) return;
  if(origem==='cartao'){
    const parcelas = DB.get('parcelas');
    const p = parcelas.find(x=>x.id===rawId);
    if(p){ p.vencimento = novaData; DB.save('parcelas', parcelas); }
    if(currentView==='cartoes') renderListaCartoes();
  } else if(origem==='fixa'){
    const parcelas = DB.get('parcelas_fixas');
    const p = parcelas.find(x=>x.id===rawId);
    if(p){ p.vencimento = novaData; DB.save('parcelas_fixas', parcelas); }
    if(currentView==='contasfixas') renderListaContasFixas();
  }
  renderNotificacoes();
  renderDashboard();
...
function dispararNotificacoesNavegador(){
  ...
  const urgentes = alertasVisiveis().filter(a=>a.status==='vencida'||a.status==='hoje');
...
/* ================================================================
   ...
   ================================================================ */
/* ================================================================
   SAÚDE FINANCEIRA + MASCOTE
   Nota de 0 a 100 combinando: contas pagas x vencidas, uso de cartão,
   receita x despesa, economia do mês e dívidas em aberto. Tudo
   calculado só com dados reais já existentes no app.
   ================================================================ */
function calcularSaudeFinanceira(d){
  let pontos = 0;
  const detalhes = [];

  // Contas pagas x vencidas no período (peso 25)
  const periodo = getPeriodoAtual();
  const parcelasPeriodo = [...DB.get('parcelas'), ...DB.get('parcelas_fixas')].filter(p=> dataDentroDoPeriodo(p.vencimento, periodo));
  if(parcelasPeriodo.length>0){
    const pagas = parcelasPeriodo.filter(p=>p.status==='pago').length;
    pontos += (pagas / parcelasPeriodo.length) * 25;
    detalhes.push({emoji:'🧾', label:'Contas pagas no período', valor:`${pagas} de ${parcelasPeriodo.length}`});
  } else { pontos += 25; detalhes.push({emoji:'🧾', label:'Contas no período', valor:'Nenhuma pendência'}); }

  // Uso do limite dos cartões de crédito (peso 20) — quanto menos usado, melhor
  const cartoesCredito = DB.get('cartoes').filter(c=>c.tipo!=='debito');
  if(cartoesCredito.length>0){
    const usoMedio = cartoesCredito.reduce((s,c)=> s + (c.limite>0 ? (c.limite-c.limiteDisponivel)/c.limite : 0), 0) / cartoesCredito.length;
    pontos += (1-Math.min(1,usoMedio)) * 20;
    detalhes.push({emoji:'💳', label:'Uso médio dos cartões', valor:`${Math.round(usoMedio*100)}%`});
  } else { pontos += 20; detalhes.push({emoji:'💳', label:'Cartões de crédito', valor:'Nenhum cadastrado'}); }

  // Receita x despesa do período (peso 25)
  if(d.receitaMes>0){
    pontos += Math.max(0, Math.min(1, (d.receitaMes-d.gastoMes)/d.receitaMes)) * 25;
  } else pontos += d.gastoMes>0 ? 5 : 25;
  detalhes.push({emoji:'⚖️', label:'Receitas x despesas', valor:`${money(d.receitaMes)} / ${money(d.gastoMes)}`});

  // Economia em relação à média dos meses anteriores (peso 15)
  if(d.mediaGastoMesesAnteriores>0){
    const variacao = (d.mediaGastoMesesAnteriores - d.gastoMes) / d.mediaGastoMesesAnteriores;
    pontos += Math.max(0, Math.min(1, 0.5+variacao)) * 15;
    detalhes.push({emoji:'📉', label:'Gasto vs. média anterior', valor: (variacao>=0?'-':'+') + Math.abs(Math.round(variacao*100))+'%'});
  } else { pontos += 15*0.7; detalhes.push({emoji:'📉', label:'Gasto vs. média anterior', valor:'Sem histórico ainda'}); }

  // Dívidas com pessoas em aberto, relativas à receita (peso 15)
  if(d.receitaMes>0){
    const proporcao = Math.min(1, d.devoOutros / (d.receitaMes*3 || 1));
    pontos += (1-proporcao) * 15;
  } else pontos += d.devoOutros>0 ? 5 : 15;
  detalhes.push({emoji:'🤝', label:'Dívidas em aberto (que você deve)', valor: money(d.devoOutros)});

  return { nota: Math.round(pontos), detalhes };
}

function classificarSaude(nota){
  if(nota>=85) return {label:'Excelente', emoji:'🟢', mascote:'🤩', cor:'var(--green)'};
  if(nota>=70) return {label:'Boa', emoji:'🟢', mascote:'😊', cor:'var(--green)'};
  if(nota>=50) return {label:'Moderada', emoji:'🟡', mascote:'😐', cor:'var(--sun-deep)'};
  if(nota>=30) return {label:'Ruim', emoji:'🟠', mascote:'😟', cor:'#E8944A'};
  return {label:'Crítica', emoji:'🔴', mascote:'😢', cor:'var(--red)'};
}

function gerarMensagemMascote(d, nota){
  const mensagens = [];
  if(d.mediaGastoMesesAnteriores>0 && d.gastoMes < d.mediaGastoMesesAnteriores*0.9){
    mensagens.push('Parabéns! Você economizou mais neste período. 🎉');
  }
  const cats = Object.entries(d.porCategoria||{});
  let maiorAumento = null;
  cats.forEach(([cat,val])=>{
    const anterior = (d.porCategoriaMesAnterior||{})[cat] || 0;
    if(anterior>0 && val > anterior*1.2){
      const pct = Math.round(((val-anterior)/anterior)*100);
      if(!maiorAumento || pct>maiorAumento.pct) maiorAumento = {cat, pct};
    }
  });
  if(maiorAumento) mensagens.push(`Seu gasto com ${maiorAumento.cat} aumentou ${maiorAumento.pct}%.`);
  (d.cartoes||[]).forEach(c=>{
    if(c.limite>0 && (c.limite-c.limiteDisponivel)/c.limite >= 0.8){
      mensagens.push(`Seu cartão ${c.nome} já usou ${Math.round(((c.limite-c.limiteDisponivel)/c.limite)*100)}% do limite.`);
    }
  });
  if(mensagens.length===0){
    mensagens.push(nota>=70 ? 'Continue assim! 🌸' : 'Vamos organizar juntos as próximas contas.');
  }
  return mensagens[0];
}

let saudeDetalhesAbertos = false;
function renderSaudeFinanceira(d){
  const { nota, detalhes } = calcularSaudeFinanceira(d);
  const c = classificarSaude(nota);
  document.getElementById('mascotePorquinho').textContent = '🐷' + c.mascote;
  document.getElementById('saudeNota').textContent = nota;
  document.getElementById('saudeNota').style.color = c.cor;
  document.getElementById('saudeClassificacao').textContent = `${c.emoji} ${c.label}`;
  document.getElementById('saudeMensagem').textContent = gerarMensagemMascote(d, nota);

  const detalhesEl = document.getElementById('saudeDetalhes');
  detalhesEl.innerHTML = detalhes.map(item=>`<div class="resumo-item"><span class="tag">${item.emoji} ${item.label}</span><span class="amt" style="font-size:12.5px;">${item.valor}</span></div>`).join('');
  detalhesEl.classList.toggle('hidden', !saudeDetalhesAbertos);
  document.getElementById('saudeSeta').textContent = saudeDetalhesAbertos ? '▲' : '▶';
}

document.getElementById('saudeFinanceiraPanel').addEventListener('click', ()=>{
  saudeDetalhesAbertos = !saudeDetalhesAbertos;
  document.getElementById('saudeDetalhes').classList.toggle('hidden', !saudeDetalhesAbertos);
  document.getElementById('saudeSeta').textContent = saudeDetalhesAbertos ? '▲' : '▶';
});
