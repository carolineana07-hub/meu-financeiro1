/* ================================================================
   ...
   - contas_fixas       { id, nome, valor, diaVencimento, categoria }
   ...
   - aprendizado_categorias, pagamentos_contas_fixas, historico_patrimonio, aniversarios
   ================================================================ */
const DB = {
...
function calcularDashboard(){
  ...
  const mesAnoAtualCalc = anoAtual + '-' + String(mesAtual+1).padStart(2,'0');
  const pagamentosFixas = DB.get('pagamentos_contas_fixas');
  contasFixas.forEach(cf=>{
    const jaPaga = pagamentosFixas.some(p=>p.contaFixaId===cf.id && p.mesAno===mesAnoAtualCalc);
    if(jaPaga) return;
    const venc = new Date(anoAtual, mesAtual, cf.diaVencimento||1);
    if(venc < hoje) contasVencidas += cf.valor||0;
    else contasAVencer += cf.valor||0;
  });
...
function abrirModalContaFixa(){
  ...
  document.getElementById('cfDiaVencimento').value='';
...
document.getElementById('btnSalvarContaFixa').addEventListener('click', ()=>{
  ...
  const diaVencimento = parseInt(document.getElementById('cfDiaVencimento').value);
  if(!nome || !valor || valor<=0 || !diaVencimento){ alert('Preencha nome, valor e dia de vencimento.'); return; }
  DB.add('contas_fixas', { nome, valor, diaVencimento, categoria: document.getElementById('cfCategoria').value });
...
function renderListaContasFixas(){
  ...
  const pagamentos = DB.get('pagamentos_contas_fixas');
  const catMeta = DB.get('categorias');
  const hoje = new Date();
  list.innerHTML = contasFixas.map(cf=>{
    const paga = pagamentos.some(p=>p.contaFixaId===cf.id && p.mesAno===mesAno);
    const venc = new Date(hoje.getFullYear(), hoje.getMonth(), cf.diaVencimento||1);
    const status = paga ? '✅ Paga este mês' : (venc < hoje ? '⚠️ Vencida' : '📅 A vencer dia ' + cf.diaVencimento);
    ...
    return `<div class="tx-item">
      <div class="tx-left">
        ...
        <div class="tx-info">
          ...
          <div class="tx-meta">${status}</div>
        </div>
      </div>
      <div class="tx-right" style="flex-direction:column; align-items:flex-end; gap:4px;">
        <div class="tx-value neg">${money(cf.valor)}</div>
        <button class="tx-del" data-id="${cf.id}" data-action="${paga?'desfazer':'pagar'}" style="font-size:11px; color:var(--rose-deep); font-weight:700;">${paga?'Desfazer':'Marcar paga'}</button>
      </div>
    </div>`;
  }).join('');
  list.querySelectorAll('[data-action]').forEach(btn=>{
    btn.addEventListener('click', ()=> togglePagamentoContaFixa(btn.dataset.id, btn.dataset.action));
  });
}

function togglePagamentoContaFixa(id, action){
  const mesAno = mesAnoAtual();
  let pagamentos = DB.get('pagamentos_contas_fixas');
  if(action==='pagar'){
    pagamentos.push({ id: Date.now().toString(36), contaFixaId:id, mesAno });
    DB.save('pagamentos_contas_fixas', pagamentos);
    const cf = DB.get('contas_fixas').find(c=>c.id===id);
    if(cf){
      DB.add('transacoes', { tipo:'despesa', valor:cf.valor, categoria:cf.categoria, data:new Date().toISOString().slice(0,10), cartaoId:null, descricao:cf.nome });
    }
  } else {
    DB.save('pagamentos_contas_fixas', pagamentos.filter(p=>!(p.contaFixaId===id && p.mesAno===mesAno)));
  }
  renderListaContasFixas();
  renderDashboard();
}
...
function coletarEventosDoDia(dateStr){
  ...
  DB.get('contas_fixas').filter(cf=>cf.diaVencimento===dia).forEach(cf=>{
    const mesAno = ano+'-'+String(mes).padStart(2,'0');
    const paga = DB.get('pagamentos_contas_fixas').some(p=>p.contaFixaId===cf.id && p.mesAno===mesAno);
    if(paga) return;
    eventos.push({ tipo:'conta', label:cf.nome, valor:cf.valor, cor: diaData<hoje?'var(--red)':'var(--rose-deep)' });
  });
...
function coletarAlertas(){
  ...
  const mesAno = mesAnoAtual();
  DB.get('contas_fixas').forEach(cf=>{
    const paga = DB.get('pagamentos_contas_fixas').some(p=>p.contaFixaId===cf.id && p.mesAno===mesAno);
    if(paga) return;
    const anoRef = hoje.getFullYear(), mesRef = hoje.getMonth();
    const vencStr = anoRef+'-'+String(mesRef+1).padStart(2,'0')+'-'+String(cf.diaVencimento||1).padStart(2,'0');
    const st = classificar(vencStr);
    if(st) alertas.push({ id:'fixa-'+cf.id+mesAno, status:st, texto:cf.nome, valor:cf.valor });
  });
...
const COLECOES_BACKUP = [
  'contas_bancarias','cartoes','transacoes','parcelas','contas_fixas',
  'dividas','parcelas_divida','caixinhas','investimentos','categorias',
  'pagamentos_contas_fixas','historico_patrimonio','aniversarios'
];
...
function responderPergunta(perguntaOriginal){
  ...
  if(p.includes('vence') && (p.includes('semana') || p.includes('proximos dias'))){
    ...
    DB.get('contas_fixas').forEach(cf=>{
      const v = new Date(hoje.getFullYear(), hoje.getMonth(), cf.diaVencimento||1);
      const paga = DB.get('pagamentos_contas_fixas').some(pg=>pg.contaFixaId===cf.id && pg.mesAno===mesAnoAtual());
      if(!paga && v>=hoje && v<=limite) itens.push(`${cf.nome} — ${money(cf.valor)} (${v.toLocaleDateString('pt-BR')})`);
    });
